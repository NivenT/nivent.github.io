...
<div class="proof4">
    We'll do the classic analysis thing and prove that the Euler product is at most $\zeta(s)$, and that $\zeta(s)$ is at least $\zeta(s)$.
    <br>
...
Our first goal is to relate these characters to $\mbf1_a$, our real function of interest. First, note that any character $\chi:\units{(\zmod N)}\to\C$ can be extended to a function $\Z\to S^1$ via
...
<div class="remark">
    Restricting a character to a subgroup defines a homomorphism $\rho:\hat G\to\hat H$. The above proposition says that $\rho$ is surjective. Since the kernel of $\rho$, characters of $G$ which are trivial on $H$, is isomorphism to $\wh{G/H}$, we obtain a short exact sequence
...
<div class="proof4">
    ...
    where the middle vertical map is well-defined because of the existence of the other two maps. Furthermore, this middle map is an isomorphism by the snake lemma or by an explicit diagram chase.
</div>
...
<div class="proof4">
    ...
    The term in the parantheses is $0$ if $\inv an\not\equiv1\pmod N$ and is $1$ if $\inv an\equiv1\pmod N$. Hence we win.
</div>
...
<div class="definition">
    A character $\chi\in X(N)$ is called <b>primitive</b> if does not exist an $N'< N$ and character $\chi'\in X(N')$ such that $\chi(n)=\chi'(\bar n)$ where $\bar n\in\zmod{N'}$ denotes $n$'s reduction modulo $N'$.
</div>
...
Before moving on, fix $\eps\in\bits$ such that $\chi(-1)=(-1)^{\eps}$. We let this $\eps$ the <b>exponent</b> of $\chi$. We use it to define the $\chi$-Gamma function
...
where the last equality comes from the fact that $\log L(\chi,1)=\infty$ iff $\chi$ is the trivial character! Thus, we've proben the following.

<div class="theorem" name="Dirichlet's Theorem on Primes in Arithmetic Progressions">
    Fix some $a,N$ with $\gcd(a,N)$. Then, there are infinitely many primes $p$ such that $p\equiv a\pmod N$. Equivalently, the arithmetic progression
...
One can use $\zeta_K$ to prove my earlier claim about the holomorphicity and non-vanishing of $L$-functions attached to nontrivial characters at $s=1$. This follows as a corollary of (a stronger verions of) the following.
...
We want to relate this splitting behavior to the Euler product for $\zeta_K(s)$. Consider an odd prime $p\in\Z$. If $p$ is inert, this it will contribute

$$\frac1{1-p^{-2s}}=\frac1{1-p^{-s}}\frac1{1+p^{-s}}$$

to $\zeta_K(s)$. If $p$ splits, then it will contribute

$$\frac1{(1-p^{-s})^2}=\frac1{1-p^{-s}}\frac1{1-p^{-s}}$$

to $\zeta_K(s)$. Finally, if $p$ is ramified, then it will contribute
