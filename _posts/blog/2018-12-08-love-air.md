...
[^14]: TODO: Prove this. That binomial there makes this statement potentially nontrivial (it should certainly be true for large g though, and that's all we need). Maybe you can do this by replacing the factorials in (2g)C(2a) with calls to the Gamma function, and then taking derivatives. UPDATE FROM FAR IN THE FUTURE: The actual way to show this is to realize that $$f(a,g)=f(g-a,g)$$ and then to show that $$f(a+1,g)/f(a,g)<1$$ when $$a<g/2$$. Also, while I'm making edits in hindsight, there's was no need to use Stirling before. One can replace $$f(a,g)$$ with the expression $$\binom ga^4\left/\binom{2g}{2a}^3\right.$$.
